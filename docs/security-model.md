...
## P1-3: IP名寄せ（link_anonymous_reviews_by_ip）の止血

IP による名寄せ（`link_anonymous_reviews_by_ip`）は、自己申告IPを信じるため
他人の匿名レビューを乗っ取れる経路があった（深刻度: 高）。2026-07 に本体を無効化
（常に0を返す）して止血。恒久対処として、投稿フロー再設計（匿名投稿のメール認証
必須化＋アカウントの検証済みメールをサーバー側でハッシュ照合する名寄せ、または
DROP）を次回検討する。それまでの扱い: 止血済み（名寄せ機能は停止中）。
